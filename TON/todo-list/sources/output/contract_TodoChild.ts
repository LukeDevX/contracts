...
async function TodoChild_init(parent: Address, seqno: bigint) {
    const __code = Cell.fromBase64('');
    const __system = Cell.fromBase64('');
...
const TodoChild_getters: ABIGetter[] = [
    ...
    {"name":"getSeqno","arguments":[],"returnType":{"kind":"simple","type":"int","optional":false,"format":257}},
    {"name":"getTask","arguments":[],"returnType":{"kind":"simple","type":"string","optional":false}},
]

export const TodoChild_getterMapping: { [key: string]: string } = {
    'details': 'getDetails',
    'getSeqno': 'getGetSeqno',
    'getTask': 'getGetTask',
}
...
export class TodoChild implements Contract {
    ...
    async getGetSeqno(provider: ContractProvider) {
        let builder = new TupleBuilder();
        let source = (await provider.get('getSeqno', builder.build())).stack;
        let result = source.readBigNumber();
        return result;
    }
    
    async getGetTask(provider: ContractProvider) {
        let builder = new TupleBuilder();
        let source = (await provider.get('getTask', builder.build())).stack;
