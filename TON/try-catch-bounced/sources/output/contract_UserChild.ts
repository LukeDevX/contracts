...
function dictValueParserUserChild$Data(): DictionaryValue<UserChild$Data> {
 ...
 type UserChild_init_args = {
    $$type: 'UserChild_init_args';
    parent: Address;
    userAddress: Address;
}

function initUserChild_init_args(src: UserChild_init_args) {
    return (builder: Builder) => {
        let b_0 = builder;
        b_0.storeAddress(src.parent);
        b_0.storeAddress(src.userAddress);
...
async function UserChild_init(parent: Address, userAddress: Address) {
    const __code = Cell.fromBase64('');
    const __system = Cell.fromBase64('');
    let builder = beginCell();
    builder.storeRef(__system);
    builder.storeUint(0, 1);
    initUserChild_init_args({ $$type: 'UserChild_init_args', parent, userAddress })(builder);
    const __data = builder.endCell();
    return { code: __code, data: __data };
}

const UserChild_errors: { [key: number]: { message: string } } = {
...
const UserChild_getters: ABIGetter[] = [
    {"name":"getAddress","arguments":[],"returnType":{"kind":"simple","type":"address","optional":false}},
]

export const UserChild_getterMapping: { [key: string]: string } = {
...
export class UserChild implements Contract {
    ...
    static async init(parent: Address, userAddress: Address) {
        return await UserChild_init(parent, userAddress);
    }
    
    static async fromInit(parent: Address, userAddress: Address) {
        const init = await UserChild_init(parent, userAddress);
        const address = contractAddress(0, init);
        return new UserChild(address, init);
    }
    
    static fromAddress(address: Address) {
        return new UserChild(address);
    }
    
    readonly address: Address; 
    readonly init?: { code: Cell, data: Cell };
    readonly abi: ContractABI = {
        types:  UserChild_types,
        getters: UserChild_getters,
        receivers: UserChild_receivers,
        errors: UserChild_errors,
    };
