...
var _ContractExecutor_state, _ContractExecutor_balance, _ContractExecutor_last, _ContractExecutor_lock, _ContractExecutor_index, _ContractExecutor_verbosity, _ContractExecutor_name;
...
function bigIntToBuffer(v, bytes) {
    return (0, core_1.beginCell)().storeUint(v, bytes * 8).endCell().beginParse().loadBuffer(bytes);
}
/**
...
class ContractExecutor {
    ...
    constructor(state, system) {
        ...
        _ContractExecutor_verbosity.set(this, null);
        _ContractExecutor_name.set(this, null);
        /**
        ...
        this.get = async (method, stack) => {
            return await __classPrivateFieldGet(this, _ContractExecutor_lock, "f").inLock(async () => {
                ...
                if ((0, coverage_1.isCoverageEnabled)()) {
                    verbosity = Math.max(3 /* Verbosity.INFO */, verbosity); // Force warning verbosity for coverage
                }
                ...
                // Report coverage
                if ((0, coverage_1.isCoverageEnabled)()) {
                    (0, coverage_1.reportCoverage)(result.vmLogs);
                }
                // Parse result
                let resultStack = (0, core_1.parseTuple)(core_1.Cell.fromBoc(Buffer.from(result.output.stack, 'base64'))[0]);
                return {
                    success: true,
                    gasUsed: BigInt(result.output.gas_used),
                    stack: new core_1.TupleReader(resultStack),
     ...
     */
    get name() {
        return __classPrivateFieldGet(this, _ContractExecutor_name, "f");
    }
    /**
     * Sets optional address name
     */
    set name(v) {
        __classPrivateFieldSet(this, _ContractExecutor_name, v, "f");
    }
    /**
     * Returns contract verbosity level
     */
    get verbosity() {
        return __classPrivateFieldGet(this, _ContractExecutor_verbosity, "f");
    }
    /**
     * Sets contract verbosity level
     */
    set verbosity(v) {
        __classPrivateFieldSet(this, _ContractExecutor_verbosity, v, "f");
    }
    /**
    ...
    async receive(msg) {
        return await __classPrivateFieldGet(this, _ContractExecutor_lock, "f").inLock(async () => {
            ...
            let verbosity = this.verbosity !== null ? this.verbosity : this.system.globalVerbosity;
            if ((0, coverage_1.isCoverageEnabled)()) {
                verbosity = Math.max(3 /* Verbosity.INFO */, verbosity); // Force debug verbosity for coverage
            }
            // Execute transaction
            let res = await this.system.bindings.transaction({
                ...
                libs: null,
                verbosity: verbosity,
                shardAccount: (0, core_1.beginCell)().store((0, core_1.storeShardAccount)({ account: __classPrivateFieldGet(this, _ContractExecutor_state, "f"), lastTransactionHash: __classPrivateFieldGet(this, _ContractExecutor_last, "f").hash, lastTransactionLt: __classPrivateFieldGet(this, _ContractExecutor_last, "f").lt })).endCell(),
                message: (0, core_1.beginCell)().store((0, core_1.storeMessage)(msg)).endCell(),
                now: this.system.now,
                lt: this.system.lt,
                randomSeed: Buffer.alloc(32),
            });
            ...
            // Report coverage
            if ((0, coverage_1.isCoverageEnabled)()) {
                (0, coverage_1.reportCoverage)(res.vmLogs);
            }
            // Apply changes
            if (res.output.success) {
                let src = core_1.Cell.fromBoc(Buffer.from(res.output.shard_account, 'base64'))[0];
                let shard = (0, core_1.loadShardAccount)(src.beginParse());
                if (shard.account) {
                ...
                // Load transaction
                let t = (0, core_1.loadTransaction)(core_1.Cell.fromBoc(Buffer.from(res.output.transaction, 'base64'))[0].beginParse());
                return {
...
_ContractExecutor_state = new WeakMap(), _ContractExecutor_balance = new WeakMap(), _ContractExecutor_last = new WeakMap(), _ContractExecutor_lock = new WeakMap(), _ContractExecutor_index = new WeakMap(), _ContractExecutor_verbosity = new WeakMap(), _ContractExecutor_name = new WeakMap();
